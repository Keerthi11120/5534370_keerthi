...
int hackerlandRadioTransmitters(int x_count, int* x, int k) {
    qsort(x, x_count, sizeof(int), cmpfunc); // Sort house positions
    ...
    while (i < x_count) {
        ...
        // Step 1: move right as far as possible within k
        int loc = x[i] + k;
        while (i < x_count && x[i] <= loc) {
            i++;
        }

        // Step 2: place transmitter at the furthest possible house
        loc = x[i - 1] + k;
