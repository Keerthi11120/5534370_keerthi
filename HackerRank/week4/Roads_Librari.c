...
long roadsAndLibraries(int n, int c_lib, int c_road, int cities_rows, int cities_columns, int** cities) {
    ...
    for (int i = 0; i < cities_rows; i++) {
        ...
        if (adj_cap[u] == adj_size[u]) {
            adj_cap[u] = adj_cap[u] ? adj_cap[u]*2 : 2;
            adj[u] = realloc(adj[u], adj_cap[u] * sizeof(int));
        }
        adj[u][adj_size[u]++] = v;

        if (adj_cap[v] == adj_size[v]) {
            adj_cap[v] = adj_cap[v] ? adj_cap[v]*2 : 2;
    ...
    for (int i = 1; i <= n; i++) {
        if (!visited[i]) {
            ...
            total_cost += (long)(comp_size - 1) * c_road; // roads
        }
    }
