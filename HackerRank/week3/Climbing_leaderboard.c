...
int* climbingLeaderboard(int ranked_count, int* ranked, int player_count, int* player, int* result_count) {
    // Step 1: Remove duplicates from ranked scores
    ...
    *result_count = player_count;
    ...
    // Step 3: Compute player ranks (player scores are in ascending order)
    int idx = unique_count - 1; // start from the lowest rank
    for (int i = 0; i < player_count; i++) {
        int score = player[i];
        // move up the leaderboard as long as player beats the score
        while (idx >= 0 && score >= unique[idx]) {
            idx--;
        }
        // if idx == -1, player is at the top rank
        result[i] = idx + 2;
    }
