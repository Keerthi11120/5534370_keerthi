...
int* waiter(int number_count, int* number, int q, int* result_count) {
    ...
    // Result array (at most number_count elements)
    int* result = malloc(number_count * sizeof(int));
    int resIndex = 0;

    // Current working stack (initially numbers)
    int* current = malloc(number_count * sizeof(int));
    int currSize = number_count;
    ...
    for (int i = 0; i < q; i++) {
        ...
        int* A = malloc(currSize * sizeof(int));
        int* B = malloc(currSize * sizeof(int));
        int aTop = 0, bTop = 0;

        // Pop from current (top is last element)
        ...
        // Push A (top to bottom) into result
        for (int j = aTop - 1; j >= 0; j--) {
            result[resIndex++] = A[j];
        }

        // B becomes new current
        free(current);
        current = malloc(bTop * sizeof(int));
    ...
    // Add remaining numbers in current (top to bottom)
    for (int j = currSize - 1; j >= 0; j--) {
        result[resIndex++] = current[j];
    }
